-- Canceled Google Play subscriptions retain access until their verified expiry.
-- Active/grace legacy records without an expiry remain compatible; canceled records
-- must always have a future expiry to receive access.
...
begin
  ...
  if not exists (
    ...
    where user_id = p_user_id
      and (
        (status in ('active', 'grace_period') and (expiry_time is null or expiry_time > now()))
  ...
  v_day_start := date_trunc(
    'day', now() + make_interval(mins => p_reset_offset_minutes)
