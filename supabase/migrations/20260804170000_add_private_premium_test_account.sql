-- Give explicitly approved production test accounts stable premium access
-- without pretending that a Google Play purchase was verified.
-- This table is service-role-only and does not bypass Voice usage limits.
create table if not exists public.premium_test_accounts (
  user_id uuid primary key references auth.users(id) on delete cascade,
  reason text not null default 'production testing',
  created_at timestamptz not null default now()
);

alter table public.premium_test_accounts enable row level security;
revoke all privileges on table public.premium_test_accounts from public, anon, authenticated;
grant select on table public.premium_test_accounts to service_role;
...
declare
  v_daily_used_minutes integer; v_monthly_used_minutes integer;
  v_reserved_minutes integer; v_day_start timestamptz; v_month_start timestamptz;
begin
  ...
  v_day_start := date_trunc('day', now() + make_interval(mins => p_reset_offset_minutes)) - make_interval(mins => p_reset_offset_minutes);
  v_month_start := date_trunc('month', now() + make_interval(mins => p_reset_offset_minutes)) - make_interval(mins => p_reset_offset_minutes);
  select coalesce(sum(reserved_minutes), 0)::integer into v_daily_used_minutes from private.voice_session_leases where user_id = p_user_id and started_at >= v_day_start;
  if v_daily_used_minutes >= p_daily_minutes then raise exception 'Daily voice allowance reached' using errcode = 'P0001'; end if;
  select coalesce(sum(reserved_minutes), 0)::integer into v_monthly_used_minutes from private.voice_session_leases where user_id = p_user_id and started_at >= v_month_start;
  if v_monthly_used_minutes >= p_monthly_minutes then raise exception 'Monthly voice allowance reached' using errcode = 'P0001'; end if;
  v_reserved_minutes := least(p_max_minutes, p_daily_minutes - v_daily_used_minutes, p_monthly_minutes - v_monthly_used_minutes);
...
language plpgsql security definer set search_path = '' as $$
declare
  v_active private.voice_session_leases%rowtype; v_daily_used_minutes integer;
  v_monthly_used_minutes integer; v_day_start timestamptz; v_month_start timestamptz;
begin
  if p_user_id is null or p_max_minutes not between 1 and 15
    or p_daily_minutes not between p_max_minutes and 240
    or p_monthly_minutes not between p_daily_minutes and 1440
  ...
  select coalesce(sum(reserved_minutes), 0)::integer into v_daily_used_minutes from private.voice_session_leases where user_id = p_user_id and started_at >= v_day_start;
  select coalesce(sum(reserved_minutes), 0)::integer into v_monthly_used_minutes from private.voice_session_leases where user_id = p_user_id and started_at >= v_month_start;
  select * into v_active from private.voice_session_leases where user_id = p_user_id and ended_at is null and expires_at > now() order by started_at desc limit 1;
  ...
  if v_daily_used_minutes >= p_daily_minutes then
    return query select true, false, 'daily_limit'::text, ceil(extract(epoch from (v_day_start + interval '1 day' - now())))::integer, false, v_monthly_used_minutes, greatest(0, p_monthly_minutes - v_monthly_used_minutes), p_monthly_minutes, v_month_start + interval '1 month';
    return;
  end if;
  if v_monthly_used_minutes >= p_monthly_minutes then
    return query select true, false, 'monthly_limit'::text, ceil(extract(epoch from (v_month_start + interval '1 month' - now())))::integer, false, v_monthly_used_minutes, greatest(0, p_monthly_minutes - v_monthly_used_minutes), p_monthly_minutes, v_month_start + interval '1 month';
    return;
  end if;
...
-- Add approved accounts separately with a service-role migration operation.
