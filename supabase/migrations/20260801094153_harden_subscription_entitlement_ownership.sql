...
alter table public.subscription_entitlements
  ...
  check (status in ('active', 'grace_period', 'on_hold', 'paused', 'canceled', 'expired', 'revoked'));

create unique index if not exists subscription_entitlements_order_id_unique_idx
  on public.subscription_entitlements (order_id)
  where order_id is not null and length(trim(order_id)) > 0;
