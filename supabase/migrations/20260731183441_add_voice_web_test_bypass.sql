-- Temporary, server-controlled web test bypass. The boolean is accepted only
-- by the service-role RPC calls; public and authenticated roles cannot invoke
-- these overloads. Vercel decides when it may be true.
...
create function public.acquire_voice_session_lease(
  ...
  p_handle_hash text,
  p_allow_payment_bypass boolean
)
...
begin
  ...
  if not coalesce(p_allow_payment_bypass, false) and not exists (
  ...
  v_day_start := date_trunc('day', now() + make_interval(mins => p_reset_offset_minutes))
    - make_interval(mins => p_reset_offset_minutes);
  v_month_start := date_trunc('month', now() + make_interval(mins => p_reset_offset_minutes))
    - make_interval(mins => p_reset_offset_minutes);

  select coalesce(sum(reserved_minutes), 0)::integer into v_daily_used_minutes
  from private.voice_session_leases where user_id = p_user_id and started_at >= v_day_start;
...
begin
  ...
  v_eligible := coalesce(p_allow_payment_bypass, false) or exists (
    select 1 from public.subscription_entitlements
    where user_id = p_user_id
      and status in ('active', 'grace_period')
      and (expiry_time is null or expiry_time > now())
  ...
  if v_active.id is not null then
    if p_handle_hash is not null and v_active.handle_hash = p_handle_hash
      and v_active.renewal_count < 2 and v_active.expires_at > now() + interval '30 seconds' then
      return query select true, true, 'reservation_resume'::text,
        ...
        p_monthly_minutes, v_month_start + interval '1 month';
    else
