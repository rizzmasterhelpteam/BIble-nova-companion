...
const tokenSource = readFileSync(new URL("../api/voice/live-token.ts", import.meta.url), "utf8");
...
describe("temporary web Voice payment bypass", () => {
  it("requires both the server switch and an exact allowed browser Origin", () => {
    for (const source of [sessionSource, tokenSource]) {
  ...
  it("passes the bypass only to service-role-only RPC overloads", () => {
    ...
    expect(tokenSource).toContain("allowPaymentBypass,");
