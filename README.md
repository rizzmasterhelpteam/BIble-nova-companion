...
Every Gemini Live ephemeral token is capped to the active Supabase Voice reservation. The database reservation expiry remains the source of truth for usage duration; both token expiry and the shorter new-session start window are constrained so they cannot outlive that reservation. The client continues to use the reservation's `remainingSeconds` for its visible session timer.

Because the Live WebSocket connects directly from the browser or Capacitor app to Gemini, the server still cannot observe or absolutely enforce concurrent provider connections after token issuance. Strict concurrency enforcement would require a server-controlled proxy/transport or stronger provider-supported session-resumption enforcement.
